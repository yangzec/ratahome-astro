...
export function resolveSlugPage(
  ...
  if (type === 'page') {
    ...
    description =
      page?.description
      ?? (locale === 'zh'
        ? `查看 ${title} 的规格、用途与询样方式。`
        : `Explore denier ranges, finishes, and typical end uses for ${title}.`);
    content =
      page?.content
      ?? (locale === 'zh'
        ? `浏览 ${title} 的工艺要点，并申请色卡或样品。`
        : `Review construction notes for ${title}, then request a shade card or sample set.`);
