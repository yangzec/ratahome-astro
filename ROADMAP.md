...
| 对外文案 | **生成 / 约束 / 验收分通道** | `content/` 只写访客材料；约束变成路由与对象，不写成「不是某某站」；`validate` 对照句是探测器不是定义 |
...
成衣站「而不是面料贸易商」混入环节：`create` 只产出空骨架，没有写该句；旧 `validate` 只查与源站全等，也没有写该句，但奖励「看起来不同」。句子写在按骨架重写 `content/` 的生成环节——「不得复用面料站」与访客文案同屏，验收语言进了 `home.audiences.title`。`deploy` 只是放行。
